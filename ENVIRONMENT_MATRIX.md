...
- Use `docker-compose.property.yml` directly only for a disposable local development target, with `EA_RUNTIME_MODE=dev` and local-only credentials; it is not a production release path.
- `make deploy` invokes the unprivileged production handoff and requires an external, operation-bound signed request plus the independently installed release controller. It rejects caller Docker authority, database credentials, traffic credentials, and Compose overrides.
