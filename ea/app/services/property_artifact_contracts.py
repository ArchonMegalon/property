...
def property_artifact_provider_lanes() -> tuple[PropertyArtifactProviderLane, ...]:
    return (
        ...
        PropertyArtifactProviderLane(
            ...
            title="krpano",
            role="3D Viewer",
            allowed_use="Licensed branded panorama/viewer shell for PropertyQuarry-hosted tours.",
            forbidden_use="No fake cube fallback and no replacement of verified Matterport or 3DVista truth.",
            privacy_posture="hosted public-safe tour assets only",
            configured=_configured("KRPANO_LICENSE_DOMAIN", "KRPANO_LICENSE_KEY"),
            live_tested_env="PROPERTYQUARRY_KRPANO_LIVE_SMOKE=1",
            fail_closed_rule="If the licensed viewer is required, missing or invalid license config must block delivery.",
        ),
        PropertyArtifactProviderLane(
            provider_key="pano2vr",
            title="Pano2VR",
            role="3D Viewer",
            allowed_use="Self-hosted branded panorama-tour export authoring and playback for prepared PropertyQuarry tours.",
            forbidden_use="No fake cube fallback and no replacement of verified Matterport or 3DVista truth.",
            privacy_posture="hosted public-safe export folder only",
            configured=_configured("PANO2VR_LICENSE_KEY", "PANO2VR_EMAIL"),
            live_tested_env="PROPERTYQUARRY_PANO2VR_LIVE_SMOKE=1",
            fail_closed_rule="Missing export entry or invalid export path must block delivery.",
        ),
        PropertyArtifactProviderLane(
