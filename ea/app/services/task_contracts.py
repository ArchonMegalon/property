...
class TaskContractService:
    ...
    def _builtin_contract(self, task_key: str) -> TaskContract | None:
        ...
        if normalized == "scene_video_generate":
            return TaskContract(
                ...
                evidence_requirements=("scene_video_request",),
                memory_write_policy="none",
                budget_policy_json={"class": "medium"},
                updated_at=now_utc_iso(),
                runtime_policy_json={
                    "workflow_template": "tool_then_artifact",
                    "pre_artifact_tool_name": "ea.scene_video_generate",
                    "brain_profile": "groundwork",
                    "posthoc_review_profile": "review_light",
                    "posthoc_review_required": False,
                    "fallback_brain_profile": "survival",
                    "skill_catalog_json": {
                        ...
                        "name": "scene video generate",
                        "description": "Shared EA scene-video contract for provider-selected motion renders, including Chummer runsite beats and PropertyQuarry walkthrough lanes.",
                        "memory_reads": ["scene_video_request"],
                        "memory_writes": [],
                        "tags": ["video", "scene", "mootion", "magicfit", "omagic"],
                        "input_schema_json": {
                            "type": "object",
                            "properties": {
                                "provider_key": {"type": "string", "enum": ["mootion", "magicfit", "omagic", "magic", "onemin_i2v", "onemin", "1min"]},
                                "context_kind": {"type": "string", "enum": ["scene_briefing", "property_walkthrough"]},
                                "title": {"type": "string"},
                        ...
                        "output_schema_json": {
                            ...
                            "required": ["deliverable_type", "provider_key", "render_status"],
                        },
                        "provider_hints_json": {
                            "primary": ["BrowserAct", "OMagic", "MagicFit"],
                            "notes": [
                                "magic is accepted as an alias for OMagic.",
                                "OMagic is separate from 1min and must produce its own model-upload receipt before it can satisfy an OMagic walkthrough claim.",
                                "1min is addressed explicitly as onemin_i2v and remains an image-to-video fallback/probe lane.",
                                "MagicFit, OMagic, and 1min scene briefings auto-seed a first frame when the caller does not provide one.",
                            ],
                        },
                        "tool_policy_json": {
