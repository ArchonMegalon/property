...
class PropertySurfaceScope:
    ...
    def for_section(cls, section: str) -> "PropertySurfaceScope":
        ...
        if normalized not in {"properties", "search", "shortlist", "research", "agents", "alerts", "billing", "account", "settings"}:
        ...
        return cls(
            ...
            wants_run_views=normalized in {"properties", "search", "shortlist", "research", "agents", "alerts"},
        )
...
class PropertyDecisionWorkbenchContract:
    ...
    client_results: list[object] = field(default_factory=list)
    client_selected: dict[str, object] = field(default_factory=dict)
    client_run: dict[str, object] = field(default_factory=dict)
