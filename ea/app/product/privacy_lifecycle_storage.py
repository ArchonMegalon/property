...
def _ensure_schema(database_url: str) -> bool:
    ...
    with _SCHEMA_LOCK:
        ...
        with _connect(normalized_database_url) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_property_privacy_request_idempotency
                ...
                cur.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_property_privacy_request_status_updated
...
def put_privacy_request_record(
    ...
    *,
    ...
    if str(database_url or "").strip():
        try:
            if _ensure_schema(database_url):
...
def _postgres_records(database_url: str, query: str, params: tuple[object, ...]) -> tuple[dict[str, object], ...]:
    if not _ensure_schema(database_url):
        return ()
    with _connect(database_url) as conn:
        with conn.cursor() as cur:
