...
_LOCALIZED_COPY: dict[str, dict[str, object]] = {
    "en": {
        ...
        "steps": [
            ("1. Must-haves", "PropertyQuarry first checks market, area, transaction type, property class, budget, minimum rooms or area, and explicit must-have preferences."),
            ("2. Details", "Listing pages, structured data, titles, snippets, floorplans, costs, media, and public data are normalized into comparable facts."),
            ...
            ("4. Preferences", "Nice-to-have, strong wish, avoid, and distance preferences move fit up or down. They do not exclude unless the user marked them as a must-have."),
            ...
            ("6. Results", "The final order combines personal fit, data confidence, freshness, duplicate handling, and retry status. Failed lists are retried separately."),
        ],
        "examples": [
            ("District mismatch", "A listing in 1220 stays hidden when only 1010 is selected."),
            ("Balcony nice-to-have", "If balcony is nice-to-have and missing, the listing stays visible but becomes a weaker fit."),
            ("School route looks safe", "A safer school or kindergarten route can improve fit when family preferences are active."),
        ...
        "pdf_title": "How PropertyQuarry fit works",
        "candidate_title": "Fit at a glance",
        ...
        "calculation_rows": [
            ("Start", "+50", "A home starts neutral once it fits the basic brief."),
            ("Must-haves matched", "+8", "Country, rent/buy mode, property type, budget, rooms and size do not conflict. Selected area is checked as eligibility only."),
            ("Data quality", "+10", "Floorplan, costs and a real 360/tour source make the listing easier to screen remotely."),
            ("Preferences", "+6", "Daily-life, commute and family preferences fit well enough to lift the home."),
            ("Location checked", "+0", "The selected district is an eligibility check, not a reward. Central, edge, or border position inside an allowed district does not add points; only missing or contradictory location data hurts fit."),
            ("Missing heating detail", "-8", "Heating is still unknown, so confidence drops until it is checked."),
            ("One wish missing", "-3", "A missing nice-to-have weakens fit but does not hide the home."),
            ("Open questions", "-5", "Remaining unknowns are kept as viewing questions."),
            ("Final fit", "=58", "50 + 8 + 10 + 6 + 0 - 8 - 3 - 5 = 58. The rows below show how neutral, nice-to-have, strong wish, must-have, and avoid settings change the movement."),
        ],
    },
    "de": {
...
_LOCALIZED_WEIGHT_EXPLAINER_COPY: dict[str, dict[str, object]] = {
    "en": {
        ...
        "calculation_detail_note": "The example uses the same fit ladder as the search: must-haves first, then detail quality and preferences move the order.",
        "weight_ladder_title": "How preference strength changes a delta",
        "weight_ladder_note": "A stronger preference changes the size of the fit move. It does not become a filter unless the user marks it as a must-have.",
        "weight_ladder_rows": [
            ...
            ("Must have", "gate or cap", "If the fact contradicts the must-have, the listing is hidden or capped instead of merely lowered."),
            ("Avoid", "negative move", "If the avoided condition is present, fit drops; only explicit avoid settings exclude."),
        ],
        ...
        "calculation_detail_rows": [
            ...
            ("Preferences", "+6", "Commute, daily-life and family preferences match at nice-to-have strength.", "Soft matches lift fit but never hide other homes by themselves.", "Neutral would be +0; nice-to-have gives about +6 here; strong wish would be about +12; a must-have contradiction would gate or cap."),
            ("Location checked", "+0", "The listing has candidate-level postal or district data, not only broad search text.", "Specific location data checks eligibility and nearby context. Being central, at the edge, or on a border inside an allowed district is not rewarded.", "Coarse data stays neutral; a contradicted postal code such as 1220 during a strict 1010 search excludes."),
            ...
            ("One wish missing", "-3", "A non-critical desired feature is absent or still open.", "A soft miss weakens fit only.", "Neutral would be 0; nice-to-have is about -3; strong wish about -6; must-have missing would gate or cap."),
