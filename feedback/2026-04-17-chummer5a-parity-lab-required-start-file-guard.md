...
- Tightened `tests/test_chummer5a_parity_lab_pack.py` so the current required-start-file contract is checked from the active assignment prompt while staying out of closure evidence.
- Kept `SUCCESSOR_HANDOFF_CLOSEOUT.yaml`, the published receipt, and oracle artifacts append-free because the package closure evidence is still green.
