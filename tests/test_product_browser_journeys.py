...
def test_legacy_console_property_shell_renders_match_threshold_slider() -> None:
    ...
    assert 'name="search_agent_notification_period"' in response.text
...
def test_propertyquarry_host_renders_branded_public_surfaces() -> None:
    ...
    assert "Set up an account that finds and ranks the right properties." in register.text
    assert "Finish setup and start the first search." in register.text
