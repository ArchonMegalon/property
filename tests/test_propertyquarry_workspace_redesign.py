...
def test_property_search_desktop_keeps_scroll_natural_and_map_controls_clickable() -> None:
    ...
    assert '.pqx-setup.pqx-surface-search > [data-pqx-mobile-panel="results"] {\n      max-height: none;\n      overflow: visible;' in workbench
    assert '.pqx-shell[data-pqx-surface="search"] .pqx-brief-drawer-panel {\n      max-height: none;\n      overflow: visible;' in workbench
...
def test_propertyquarry_shortlist_open_recovers_detailed_preview_for_selected_candidate(monkeypatch) -> None:
    ...
    shortlist = client.get("/app/shortlist", params={"run_id": "run-preview", "candidate": "cand-preview"}, headers=headers)
...
def test_property_research_detail_uses_minimal_top_navigation_layout() -> None:
    ...
    assert 'href="{{ research_feedback.get(\'profile_href\') or (\'/app/search\' ~ research_query_suffix) }}">Search brief</a>' in body
...
def test_propertyquarry_workspace_routes_render_greenfield_surfaces(monkeypatch) -> None:
    ...
    assert "Search brief" in packet.text
...
def test_property_run_customer_visible_events_adds_useful_synthetic_progress() -> None:
    ...
    assert "Waiting for the first provider response." in messages
    assert "Preparing 9 provider checks." in messages
...
def test_property_run_customer_visible_events_keeps_latest_ten_useful_updates() -> None:
    events = property_surface_state.property_run_customer_visible_events(
        run_payload={
            ...
            "status": "in_progress",
            "current_step": "source_fetch",
            "message": "Current provider update.",
            ...
            "events": [
                {
                    "step": "source_fetch",
                    "status": "in_progress",
                    "message": f"Provider update {index}.",
    ...
    assert "Provider update 2." not in messages
    assert "Provider update 3." in messages
    assert "Provider update 11." in messages
    assert any(message.startswith("Current provider update.") for message in messages)
...
def test_propertyquarry_selected_score_read_uses_confirmed_distance_warning(monkeypatch) -> None:
    ...
    page = client.get("/app/properties", params={"run_id": "run-distance-warning"}, headers=headers)
...
def test_propertyquarry_selected_score_read_uses_snapshot_backed_confirmed_distance_warning(monkeypatch) -> None:
    ...
    page = client.get("/app/properties", params={"run_id": "run-distance-warning-snapshot"}, headers=headers)
