...
def test_property_scout_heyy_notification_suppresses_out_of_scope_candidate(monkeypatch) -> None:
    ...
    client = build_product_client(principal_id=principal_id)
    start_workspace(client, mode="personal", workspace_name="Heyy Scout Location Guard", selected_channels=["whatsapp"])
    onboarding = client.app.state.container.onboarding
    ...
    monkeypatch.setattr(
        "app.product.service.HeyyWhatsAppBridgeService.send_template",
        lambda self, **kwargs: pytest.fail("out-of-scope WhatsApp scout alert was sent"),
    )
    repair_calls: list[dict[str, object]] = []
    monkeypatch.setattr(
        ProductService,
        "_open_property_provider_repair_task",
        lambda self, **kwargs: repair_calls.append(kwargs) or {"status": "opened", "queue_item_ref": "repair-1220"},
    )
    ...
    result = service._send_heyy_property_match_notification(
        ...
        actor="property_scout",
        template_kind="property_match",
        property_ref="property-scout:derstandard-1220",
        property_title="Wohnung mieten in 1220 Wien | 60 m² | 2 Zimmer | EUR 1.090",
        property_summary="2-Zimmer Wohnung mit Traumblick / UNO und U-Bahn ums Eck in 1220 Wien.",
        ...
        reason="shortlist",
        missing_fact="Operating costs",
        source_id="property-scout:derstandard-1220",
    )

    assert result["status"] == "suppressed"
    assert result["reason"] == "property_location_conflicts_with_active_search"
    assert result["queue_item_ref"] == "repair-1220"
    assert repair_calls
    assert repair_calls[0]["filter_key"] == "location_scope"
