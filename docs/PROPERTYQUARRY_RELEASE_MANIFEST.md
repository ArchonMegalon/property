...
- The latest 2026-07-06 live deploy now runs commit `f3292268` locally and through the PropertyQuarry release remotes; it keeps the research-detail nearby-distance backfill live, restores the generic `3D Tour` marker on the public Matterport control shell, and accepts hosted flythrough-pane walkthrough chips in the presentation gate.
...
| Branch | `main` |
| Runtime commit SHA | `f32922688622568f3f183249f7db38c9111eb296` |
| Deployment endpoint | `http://127.0.0.1:8097` with `Host: propertyquarry.com` origin smoke |
| Public domain | `https://propertyquarry.com` |
| Deployment ID | `local-20260706T210630Z-f32922688622`; current integrated local/live candidate with nearby-distance backfill on research detail, hosted walkthrough-chip gate parity, public tour-shell marker repair, public/auth shared-run smoke coverage, tour playback gates, account/billing/auth polish, OMagic adapter packaging, and explicit fail-closed scene-video provider-runtime blockers |
