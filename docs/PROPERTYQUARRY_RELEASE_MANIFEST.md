...
- Runtime commit `7698f1a4` binds an eight-row PropertyQuarry journey matrix to the exact candidate SHA. Its governed receipt passed all `7/7` source-backed cases and `8/8` real-browser cases with zero skips, while authentication activation and external notification delivery remain explicitly unevaluated until their protected launch receipts exist. The public-entry regression also proves unreadable optional tour media falls back to the homepage without exposing a filesystem path. The provider-sensitive target-recovery canary now scans the exact top-rank window it asserts instead of inheriting the 80-listing background-search budget; its 180-second terminal gate, live-provider PRESENT check, and ranking assertions remain fail-closed. This is candidate proof, not a claim that the currently stale public deployment has been corrected.
...
- The two superseded hosted repositories exposed a timing split in the same Willhaben target-recovery canary: the mirror passed in `437.13s`, while the primary crossed its fixed per-run boundary and remained `in_progress`. The canary had selected a target only from the first five provider positions but inherited the product's 80-listing scan cap. The candidate binds that scan cap to the complete asserted rank window (and selected target position), keeps the exact live-provider PRESENT pre-check and 180-second terminal assertion, and adds bounded failure diagnostics. The real Willhaben canary then passed locally in `199.11s`; its remaining 41 deterministic contracts also passed.
