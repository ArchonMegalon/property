# PropertyQuarry Release History through 2026-07-16

This archive preserves the prior detailed manifest for audit history. It is non-authoritative; current release truth lives in [`../PROPERTYQUARRY_RELEASE_MANIFEST.md`](../PROPERTYQUARRY_RELEASE_MANIFEST.md).
...
- Runtime commit `58e34fad` binds the eight-row PropertyQuarry journey matrix to the corrected candidate contract. Its governed receipt covers all `7/7` source-backed cases and `8/8` real-browser cases, while authentication activation and external notification delivery remain explicitly unevaluated until their protected launch receipts exist. The public-entry regression also proves unreadable optional tour media falls back to the homepage without exposing a filesystem path. The provider-sensitive target-recovery canary scans the asserted rank window plus one adjacent slot for no-store provider refresh volatility instead of inheriting the 80-listing background-search budget; its 180-second terminal gate, live-provider PRESENT check, and ranking assertions remain fail-closed. The real Willhaben canary passed locally in `406.45s`, including discovery. This is candidate proof, not a claim that the currently stale public deployment has been corrected.
...
| Runtime commit SHA | `58e34fad3486eca8a3dc720286efba902ffb1418` |
...
- Dual-repository hosted API receipts then consistently isolated nine deterministic regressions: the refreshed provider target moved from zero-based index four to five between discovery and the no-store probe, six deploy-guard fixtures lacked the mandatory journey matrix, and two readiness fixtures still described the superseded v1 pulse. Runtime commit `58e34fad` repairs those fixtures and the single-slot provider boundary without relaxing the rank, timeout, browser, journey, or production gates. The combined deploy/readiness regression returned `17 passed`, the remaining canary contracts returned `41 passed`, the truth-plane selection returned `7 passed`, and the real Willhaben canary returned `1 passed` in `406.45s` including discovery.
...
- A fresh isolated real-browser receipt then passed all `9/9` required route/engine samples with zero failed rows, zero engine failures, exact offline recovery, terminal decoded visible images, and no provider response fulfillment or mocking.
...
- Focused wiring regressions passed:
  ...
  - `python3 -m py_compile scripts/propertyquarry_gold_status.py tests/test_property_deploy_operator_contracts.py tests/test_propertyquarry_gold_status.py` passed.
  - `bash -n scripts/deploy_propertyquarry.sh scripts/property_release_gates.sh scripts/refresh_propertyquarry_current_gold_receipts.sh` passed.
- Live runtime truth still fails closed on the same external-state blockers: `docker exec propertyquarry-api python /app/scripts/property_scene_video_runtime_status.py --format operator` reported `ready 2/5 | blocked 3 | action_required 3`, with `magicfit`, `magic`, and `omagic` blocked.
- The refreshed `_completion/scene_video_readiness/runtime-status.json` now exists with contract `propertyquarry.scene_video_runtime_status.v1` and summary `ready_count=2`, `blocked_count=3`, `action_required_count=3`.
- A direct gold-status run with current receipts (`python3 scripts/propertyquarry_gold_status.py --write /tmp/propertyquarry-gold-status-smoke.json`) now emits nested `scene_video_readiness.runtime_status`, carries blocked providers `["magicfit", "magic", "omagic"]`, and records the runtime-status receipt path `/docker/property/_completion/scene_video_readiness/runtime-status.json`.
- `make deploy` rebuilt the live `propertyquarry-api` image and restarted `propertyquarry-api` plus `propertyquarry-scheduler`; both containers returned healthy afterward.
- `curl -fsS http://127.0.0.1:8097/version` returned `release_commit_sha=1a5f25d26c14450a3e22c7660a74f8fa56c76561` with deployment id `local-20260707T021543Z-1a5f25d26c14`.
